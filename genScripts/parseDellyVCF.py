...
class DellyVCFline(object):
    ...
    def loadVar(self):
        ...
        self.MQ = self.getValue(key="MAPQ", list=self.info)
    ...
    def printVar(self):
        ...
        outString = self.chr1 + "\t" + self.pos1  + "\t" + self.chr2 + "\t" + self.pos2 + "\t" + self.filter + "\t" + \
                    self.PEsupport +  "\t" + self.varDepth + "\t" + self.refPairs + "\t" + self.varPairs + "\t" +\
                    self.refJxReads + "\t" + self.varJxReads
        return outString

    def printTitle(self):
        title = "Chr1\tPos1\tChr2\tPos2\tCallQuality\tPairEndSupport\tVariantDepth\tReferencePairs\tVariantPairs\t" \
                "RefJunctionReads\tVarJunctionReads"
        return title
